...
int main(int argc, const char *argv[])
{
   ...
   // vector<DataFrame> dataBuffer;
            ...
            for(const auto& matcher_type : matcherTypeList)
            {
                for(const auto& selector_type : selectorTypeList)
                {
                    ...
                    for(int i = 0;i < 10 ; i++)
                    {
                        ...
                        combo.numKeyPointsPerframe[i] = 0.0f;
                        combo.numKeyPointsPerROI[i] = 0.0f;
                        combo.numMatchedKeyPoints[i] = 0.0f;
                    }
    ...
    cout << "Combinations loaded with "<<combinations.size()<<endl;
    ...
    for(auto& combo : combinations)
    {
        ...
        for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
        {
            ...
            DataFrame frame_ring;
            frame_ring.cameraImg = imgGray;
            int length_ringBuff = dataBuffer.size();
            int diff = (length_ringBuff - dataBufferSize);
            cout << "diff : "<<diff<<endl;
            if (diff > 0)
            {
            ...
            dataBuffer.push_back(frame_ring);
            ...
            vector<cv::KeyPoint> limitKeyPoints;
            if (bFocusOnVehicle)
            {
                for(auto it=keypoints.begin();it!=keypoints.end();it++)
                {
                    ...
                    {
                        limitKeyPoints.push_back(*it);

                    }
                }
            }

            keypoints = limitKeyPoints;
            combo.numKeyPointsPerROI[imgIndex] = keypoints.size();
            cout<<"No of keypoints in ROI n = "<<keypoints.size()<<endl;
            ...
            combo.descriptorType = descriptorType;
            //// EOF STUDENT ASSIGNMENT
